            ...
            filteredTokens.map((token, index) => {
              ...
              return (
                <button
                  ...
                  onClick={() => { if (!isForge) selectedToken(token); }}
                  disabled={isForge}
                >
                  ...
                  <p className="flex flex-col items-start font-medium flex-1">
                    <span className="text-[var(--clr-darker-two)] dark:text-[var(--clr-heading)] font-semibold">
                      {token.name}
                      {isWlcai && <span className="ml-2 text-xs font-normal opacity-60">wrapped LCAI</span>}
                    </span>
                  ...
                  {isForge && (
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0" style={{background:"rgba(255,140,30,.15)",color:"var(--ae-aurum)"}}>
                      {"🔥"} Trade on Forge
                    </span>
                  )}
                </button>
