    ...
    <main className="mx-auto max-w-5xl px-4 py-10 min-h-[70vh]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6">
        <div>
          ...
          {/* LCAI remaining to graduation */}
          {!coin.graduated && (
            <div className="flex justify-between text-xs mt-2" style={{ color: "var(--ae-nebula)" }}>
              <span style={{ color: "var(--ae-aurum)" }}>
                ~{fmtLcai(BigInt(Math.max(0, 300000 - Number(coin.lcaiRaised) / 1e18)) * BigInt(1e18), 0)} LCAI until graduation
              </span>
            </div>
          )}

          {/* Creator dashboard — only visible to the creator */}
          {walletAddress?.toLowerCase() === coin.creator.toLowerCase() && (
            <div className="mt-6 rounded-2xl p-5 forge-breathe"
              style={{ background: "var(--ae-haze)", border: "1px solid rgba(255,140,30,.45)" }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="ml-auto text-xs px-2 py-0.5 rounded-full font-semibold"
                  style={{ background: "rgba(255,140,30,.15)", color: "var(--ae-aurum)" }}>Creator</span>
              </div>
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="rounded-xl p-3" style={{ background: "var(--ae-night)" }}>
                  <div style={{ color: "var(--ae-nebula)" }}>Your holdings</div>
                  <div className="font-semibold mt-0.5" style={{ color: "var(--clr-heading)" }}>
                    {fmtTokens(creatorBal)} {coin.symbol}
                  </div>
                  <div style={{ color: creatorPct > 10 ? "var(--clr-warning)" : "var(--clr-success)" }}>
                    {creatorPct.toFixed(1)}% of supply
                  </div>
                </div>
                <div className="rounded-xl p-3" style={{ background: "var(--ae-night)" }}>
                  <div style={{ color: "var(--ae-nebula)" }}>LCAI raised</div>
                  <div className="font-semibold mt-0.5" style={{ color: "var(--clr-heading)" }}>
                    {fmtLcai(coin.lcaiRaised, 0)} LCAI
                  </div>
                ...
                <div className="rounded-xl p-3" style={{ background: "var(--ae-night)" }}>
                  ...
                  <div className="font-semibold mt-0.5" style={{ color: "var(--clr-heading)" }}>
                    {fmtLcai(coin.lcaiRaised / 100n, 2)} LCAI
                  </div>
                ...
                <div className="rounded-xl p-3" style={{ background: "var(--ae-night)" }}>
                  ...
                  <a href={`https://mainnet.lightscan.app/address/${coin.address}`}
                    ...
                    className="font-semibold mt-0.5 underline block truncate"
